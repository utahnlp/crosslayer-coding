...
clt_config = CLTConfig(
    ...
    decoder_tying="per_source",  # Use one decoder per source layer
    enable_feature_offset=True,  # Enable per-feature bias (feature_offset)
    enable_feature_scale=True,  # Enable per-feature scale (feature_scale)
    per_target_scale=True,  # Not using per-target adaptations
    per_target_bias=True,
...
if model.encoder_module.feature_offset is not None:
    print(f"- Feature offset parameters per layer: {len(model.encoder_module.feature_offset)}")
    print(f"- Feature offset shape (layer 0): {model.encoder_module.feature_offset[0].shape}")
else:
