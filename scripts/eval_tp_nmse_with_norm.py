    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
        for i in range(num_batches):
            try:
                ...
                if i % 10 == 0:
                    logger.info(f"Batch {i}: NMSE={metrics.get('reconstruction/normalized_mean_reconstruction_error', 0):.4f}, "
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to checkpoint directory or merged .safetensors file"
    )
    parser.add_argument(
        "--activation-path",
        type=str,
