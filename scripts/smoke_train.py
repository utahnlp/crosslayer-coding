...
def main():
    ...
    if is_distributed_run:
        ...
        logger.warning(f"Distributed Run: Using REMOTE server {server_url} for dataset '{dataset_id}'.")
        logger.warning("PREREQUISITE: Ensure the remote server is running and accessible.")
        logger.warning(f"PREREQUISITE: Ensure the dataset '{dataset_id}' is available on the server.")
    else:
        ...
        logger.info(f"Single-Process Run: Using LOCAL activations from {local_activation_path}.")
        if not Path(local_activation_path).exists():
            logger.error(f"Local activation data not found at {local_activation_path}")
            logger.error("Please ensure this path is correct and activations (for Pythia-70m) exist.")
    ...
    clt_config = CLTConfig(
        ...
        clt_dtype="bfloat16" if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else "float32",
    )

    # Ensure activation_dtype for TrainingConfig is one of the allowed string literals
    tc_activation_dtype: Literal["bfloat16", "float16", "float32"]
    if clt_config.clt_dtype == "bfloat16":
        tc_activation_dtype = "bfloat16"
    elif clt_config.clt_dtype == "float16":
        tc_activation_dtype = "float16"
    else:  # Default to float32 if None or other string
        tc_activation_dtype = "float32"
    ...
    training_config = TrainingConfig(
        ...
        activation_dtype=tc_activation_dtype,
        ...
        seed=42,
        precision="bf16" if clt_config.clt_dtype == "bfloat16" else "fp32",
    )
    ...
    # --- Run Training ---
    logger.info(f"Starting smoke training ({run_type}, WandB enabled)...")
    try:
        trainer.train()
