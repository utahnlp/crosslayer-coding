...
class CLTPatchedModel(torch.nn.Module):
    ...
    forward uses CLT reconstructions with cross-layer connections.
    """
    ...
    def __init__(self, original_model: torch.nn.Module, clt_model: CrossLayerTranscoder):
        ...
        self.layer_inputs = {}  # Stores inputs for each layer during forward pass
        self.clt_outputs_cache = {}  # Cache CLT outputs to avoid recomputation
        self.current_forward_id = 0  # Unique ID for each forward pass
    ...
    def _reset_forward_state(self):
        ...
        self.layer_inputs = {}
        self.clt_outputs_cache = {}
        self.current_forward_id += 1
        ...
        for idx, (path, mlp_mod) in enumerate(candidates):
            ...
            def make_forward(orig_fwd, layer_idx, mod_path):
                def patched_forward(*args, **kwargs):
                    ...
                    b, s, h = x_clt.shape
                    self.debug_info["activation_shapes"][f"layer_{layer_idx}"] = {
                        "batch_size": b, "seq_len": s, "hidden_size": h
                    }

                    with torch.no_grad():
                        # Store this layer's input for cross-layer processing
                        x_flat = x_clt.reshape(-1, h)
                        self.layer_inputs[layer_idx] = x_flat

                        # Check if we've already computed CLT outputs for this forward pass
                        # up to and including this layer
                        cache_key = (self.current_forward_id, layer_idx)
                        
                        if cache_key not in self.clt_outputs_cache:
                            # Gather all inputs from layer 0 up to current layer
                            clt_input = {i: self.layer_inputs[i] 
                                       for i in range(layer_idx + 1) 
                                       if i in self.layer_inputs}
                            
                            # Run CLT forward with all accumulated inputs
                            # This allows cross-layer connections to be computed
                            clt_outputs = self.clt_model.forward(clt_input)
                            
                            # Cache all outputs from this CLT call
                            for layer_id, output in clt_outputs.items():
                                self.clt_outputs_cache[(self.current_forward_id, layer_id)] = output
                        
                        # Retrieve the reconstruction for this specific layer
                        if cache_key not in self.clt_outputs_cache:
                            # no reconstruction available for this layer
                            # fall back to original
                            return orig_fwd(*args, **kwargs)

                        clt_recon = self.clt_outputs_cache[cache_key].reshape(b, s, h)
                        ...
                        self.debug_info["layer_output_differences"].append({
                            "layer": layer_idx, "mean_diff": diff, "max_diff": max_diff
                        })
                        if diff < 1e-6:
    ...
    def forward(self, *args, **kwargs):
        # Reset state at the start of each forward pass
        self._reset_forward_state()
        
        if not getattr(self, "_first_forward_done", False):
            self._first_forward_done = True
            print("\n=== FIRST FORWARD PASS (CLT patches active with cross-layer connections) ===")
    ...
    def _print_debug_summary(self):
        ...
        if self.debug_info["layer_output_differences"]:
            ...
            if avg < 1e-6:
                ...
                print("✅ CLT modifies MLP outputs with cross-layer contributions.")
...
class TranscoderPatchedModel(torch.nn.Module):
    """
    Wrap an existing causal LM and patch identified 'MLP-like' modules so their
...
def main():
    ...
    p.add_argument("--model", default="allenai/OLMo-2-0425-1B-Instruct", help="Target model name")
    p.add_argument("--dataset", default="wikitext", help="Dataset: 'name' or 'name:config'")
...
# python3 repl_eval_simple_forward.py --checkpoint "../../crosslayer-coding-v2/data/clt/allenai/OLMo-2-0425-1B-Instruct/olmo-mix-1124_train_1000000_float32/100feats_4batchsize/step_3000/model.safetensors"
# 